...
import { renderBrochureSection } from '../common/brochures/sections';
import { useTranslation } from 'react-i18next';

interface BrochureItem extends SidebarItem {}

export const BrochuresPage: React.FC = () => {
  const { t } = useTranslation('brochures');

  const items: BrochureItem[] = useMemo(() => [
  ...
  const [selected, setSelected] = useState<BrochureItem>(items[0]);

  const handleSelect = (item: BrochureItem) => {
    setSelected(item);
        ...
        <section className="max-w-[90rem] mx-auto px-8 py-16 md:py-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
            ...
            <div className="md:col-span-9 space-y-10">
              {renderBrochureSection(selected.id as any)}
            </div>
          </div>
        </section>
      </main>
