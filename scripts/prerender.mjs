...
// Pages that exist in a locale-prefixed form (/es/physicians, /zh/brochures/fertility, etc);
// see LanguageSwitcher.tsx's isLocalizable() for the same list.
const ROUTES = [
  '/',
  '/es',
  '/zh',
  ...
  '/es/conditions',
  '/zh/conditions',
  ...BROCHURE_TOPICS.map((topic) => `/brochures${topic}`),
  ...BROCHURE_TOPICS.map((topic) => `/es/brochures${topic}`),
  ...BROCHURE_TOPICS.map((topic) => `/zh/brochures${topic}`),
  '/gallery',
  '/es/gallery',
  '/zh/gallery',
  '/contact',
